...
GATES = ('throughput', 'toolcall', 'needle', 'multi')
TOOLCALL_TOKENS = 512
TOOLCALL_TOKENS_ROUND2 = 512
# Multi-needle RULER-style variant: three project codes in one haystack, one
# aggregation question requiring ALL three codes. Neutral wording (the
# proven single-needle formula: plain internal-audit records, no sharing
# restrictions that trigger compliance deliberation).
MULTI_NEEDLES = (
    {'project': 'Aurora', 'code': '7391', 'fraction': 0.15,
     'paragraph': 'Project Aurora maintains a private access code for internal audits. '
                  'The code is 7391. It is listed here for completeness of the records.'},
    {'project': 'Borealis', 'code': '4826', 'fraction': 0.45,
     'paragraph': 'Project Borealis maintains a private access code for internal audits. '
                  'The code is 4826. It is listed here for completeness of the records.'},
    {'project': 'Cascade', 'code': '9517', 'fraction': 0.75,
     'paragraph': 'Project Cascade maintains a private access code for internal audits. '
                  'The code is 9517. It is listed here for completeness of the records.'},
)
MULTI_QUESTION = ('According to the reference document above, what are the private access codes '
                  'of Project Aurora, Project Borealis and Project Cascade? Answer with the three '
                  'four-digit codes only, separated by commas.')
...
NEEDLE_CODE = '7391'
NEEDLE_TOKENS = 512
NEEDLE_TARGET_TOKENS = int(os.environ.get('NEEDLE_TARGET_TOKENS', '2048'))
NEEDLE_TOKEN_TOLERANCE = 0.10
NEEDLE_POSITIONS = {'start': 0.1, 'middle': 0.5, 'end': 0.9}
NEEDLE_REQUEST_SECONDS = int(os.environ.get('NEEDLE_REQUEST_SECONDS', '300'))
NEEDLE_WORK_SECONDS = int(os.environ.get('NEEDLE_WORK_SECONDS', '960'))
# Context slice: server context window (CTX env passed to the launcher) and
# the model-load deadline share the extended GPU-duration mandate of
# 2026-09-11 (~12:15). RAM/VRAM caps and all other guards unchanged.
SERVER_CTX = os.environ.get('SERVER_CTX', '8192')
MODEL_LOAD_DEADLINE = int(os.environ.get('MODEL_LOAD_DEADLINE', '360'))
TOOL_SCHEMA = {
    'type': 'object',
    'properties': {
...
def validate_plan(startup_nmax, checkpoint=DEFAULT_CHECKPOINT, gate='throughput'):
    ...
    if gate in ('toolcall', 'needle', 'multi'):
        if startup_nmax != 0:
...
def clean_environment(inherited, scope, startup_nmax=4, checkpoint=DEFAULT_CHECKPOINT):
    ...
    env.update(GGML_CUDA_NO_PINNED='1', DRAFT=str(int(startup_nmax != 0)), DRAFT_NMAX=str(startup_nmax),
               ...
               CTX=SERVER_CTX, NCMOE='36', NGL='99', THREADS='16', KVT='q8_0',
...
def insert_all_needles(paragraphs):
    """Insert every MULTI_NEEDLES paragraph at its fraction; positions unique
    and strictly increasing by construction (fractions are ordered)."""
    ordered = sorted(MULTI_NEEDLES, key=lambda n: n['fraction'])
    with_needles = list(paragraphs)
    positions = []
    for needle in ordered:
        index = round(len(with_needles) * needle['fraction'])
        index = min(max(index, 0), len(with_needles))
        with_needles.insert(index, needle['paragraph'])
        positions.append(index)
    return with_needles, positions


def multi_needle_payload(paragraphs):
    content = '\n\n'.join(paragraphs) + '\n\n' + MULTI_QUESTION
    return {'messages': [{'role': 'user', 'content': content}],
            'max_tokens': NEEDLE_TOKENS, 'temperature': 0.0, 'seed': 42,
            'stream': False, 'cache_prompt': True}
...
def validate_multi_needle(data):
    """Clean stop, non-empty answer; counts which codes are present."""
    message = _validated_choice(data, 'stop')
    content = message.get('content')
    if not isinstance(content, str) or not content.strip():
        raise RuntimeError('multi-needle answer is empty')
    if message.get('tool_calls') is not None:
        raise RuntimeError('multi-needle answer contains unexpected tool calls')
    recalled = sorted(needle['code'] for needle in MULTI_NEEDLES if needle['code'] in content)
    return {'recalled_codes': recalled, 'all_recalled': len(recalled) == len(MULTI_NEEDLES),
            'content': content}


def multi_needle_verdict(results):
    """PASS requires the single aggregation answer to contain ALL codes."""
    failures = []
    entry = results.get('single')
    if not isinstance(entry, dict) or not entry.get('all_recalled'):
        missing = [n['project'] for n in MULTI_NEEDLES
                   if not isinstance(entry, dict) or n['code'] not in entry.get('recalled_codes', [])]
        failures.append('aggregation answer missing: ' + ', '.join(missing))
    return {'gate': 'PASS' if not failures else 'FAIL', 'failures': failures,
            'result': dict(results)}
...
class Run:
    ...
    def __init__(self, out, startup_nmax, checkpoint=DEFAULT_CHECKPOINT, gate='throughput'):
        ...
        self.request_seconds = NEEDLE_REQUEST_SECONDS if gate in ('needle', 'multi') else REQUEST_SECONDS
        self.work_seconds = NEEDLE_WORK_SECONDS if gate in ('needle', 'multi') else WORK_SECONDS
    ...
    def execute(self):
        ...
        deadline = min(self.start + self.work_seconds, time.monotonic() + MODEL_LOAD_DEADLINE)
        ...
        for index, step in enumerate(self.sequence, 1):
            ...
            elif self.gate == 'multi':
                label = f'{index:02d}-{step}'
                if self.haystack_paragraphs is None:
                    self.haystack_paragraphs = size_haystack(self.tokenize_post)
                paragraphs, _ = insert_all_needles(self.haystack_paragraphs)
                body = multi_needle_payload(paragraphs)
            else:
            ...
            if self.gate == 'toolcall':
                ...
                data = json.loads(response_path.read_text())
                result = dict(validate_multi_needle(data), label=label, step=step,
                              wall_seconds=time.monotonic() - began,
                              prompt_tokens=data.get('usage', {}).get('prompt_tokens'),
                              completion_tokens=data.get('usage', {}).get('completion_tokens'))
            else:
